...
export default function SendUsdt({ params }: any) {
  ...
  //} , [address, contract, params.center]);
    ...
    // balance of CKEC
  const [ckecBalance, setCkecBalance] = useState(0);
  useEffect(() => {
    if (!address) {
      return;
    }
    // get the balance
    const getCkecBalance = async () => {
      const result = await balanceOf({
        contract: contractCKEC,
        address: address,
      });

      setCkecBalance( Number(result) / 10 ** 18 );

  
    };
    if (address) getCkecBalance();
    const interval = setInterval(() => {
      if (address) getCkecBalance();
    } , 5000);
    return () => clearInterval(interval);
    ...
    try {
        ...
        */
        ...
        if (transactionHash) {
          ...
          toast.error('Failed to send CKEC');
        }
      

    } catch (error) {
      toast.error('Failed to send CKEC');
      console.error('Error sending CKEC:', error);
    }
  ...
  // transfer list CKEC
  const [transferListCKEC, setTransferListCKEC] = useState([]);
  const [loadingTransferListCKEC, setLoadingTransferListCKEC] = useState(false);
  useEffect(() => {
    const getTransferListCKEC = async () => {
      setLoadingTransferListCKEC(true);
      const response = await fetch('/api/transfer/getAllTransferCKEC', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        ...
        setLoadingTransferListCKEC(false);
        return;
      }
      const data = await response.json();

      setTransferListCKEC(data.result.transfers);
                ...
                {true && (
                  <div className="w-full mt-5 bg-white rounded-lg p-4">
                      ...
                      <table className="w-full table-auto">
                        ...
                        <tbody>
                          {transferListCKEC.map((transfer : any, index: number) => (
